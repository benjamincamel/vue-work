  ...
  <div class="app-container">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item v-if="pageType === 0">新增员工信息</el-breadcrumb-item>
        <el-breadcrumb-item v-else>修改员工信息</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      <el-tabs>
...
export default {
  ...
  methods: {
    ...
    submitInfo() {
      ...
      if (!this.personalAll.personalSalaryInfo.arrivalTime) {
        this.tools.alertError(this, '请填写到岗时间！')
        return
      }
      if (!this.personalAll.personalSalaryInfo.entryTime) {
  ...
  created() {
    // 0-新建员工信息  1-编辑员工信息
    this.pageType = Number(this.$route.params.pageType)
    this.pageType === 1 && (this.userId = Number(this.$route.params.userId))
    // 如果是编辑员工信息  查询原本数据
    if (this.pageType === 1) {
      this.getData(
        'personal/getPersonalAllInfoById',
        { personalInfoId: this.$route.params.userId },
        data => {
